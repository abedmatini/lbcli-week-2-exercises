...
# =========================================================================
TX_SIZE=$((10 + 68 + 62)) # Base + 1 In + 2 Out
...
# =========================================================================
# CHALLENGE 4: Create Raw Transaction
# =========================================================================
...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r ".vout[] | select(.scriptPubKey.address == \"$PAYMENT_ADDRESS\") | .value" | awk '{printf "%.8f", $0}')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r ".vout[] | select(.scriptPubKey.address == \"$CHANGE_ADDRESS\") | .value" | awk '{printf "%.8f", $0}')
...
CHILD_TX_SIZE=$((10 + 68 + 31)) # Base + 1 In + 1 Out
